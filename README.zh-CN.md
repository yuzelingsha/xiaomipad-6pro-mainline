...
- **容量与布局**：安装器已在已知的 **256 GB 分区布局**完成真机验证，并不代表所有 256 GB 设备均兼容。其他容量、改过分区的设备未验证，不支持安装。安装器会重新划分磁盘尾部，并把 Ubuntu 安装到 B 槽；不要绕过校验。
- **验证范围**：首次安装、首次启动、旋转、触摸、磁吸键盘（含触摸板）、声音和 USB OTG 主机模式（有线鼠标）已验证；Android 恢复流程尚未完成独立真机验证。
- **安装方式**：安装器提供 Ubuntu 单系统布局与保留原厂 Android（A 槽）的双系统布局。双系统布局已在 256 GB 机型完成真机安装，Ubuntu 从 B 槽正常启动；Android 首次启动、KernelSU 与双系统切换尚未验证，单系统布局尚未在真机上安装。详见[安装步骤](docs/INSTALL-TESTING.zh-CN.md)。
...
| 后置摄像头 | S5KJN1 / Qualcomm CAMSS / libcamera 软件图像处理 | 🟡 部分支持 | 临时启动镜像已验证预览、拍照；实验性录像仍有卡顿，详见[测试范围与限制](docs/CAMERA.md) |
